...
export const getUser = (user: User): Response<User> => {
  ...
  metrics.getUserResponseTime.add(res.timings.waiting)
  logWaitingTime({
  ...
  check(res, {
    ...
    'Get User By Id: has correct email': (_) => jsonRes.data.email === user.email
  })
...
export const updateUser = (user: User): Response<UpdateUserResponse> => {
  ...
  metrics.updateUserResponseTime.add(res.timings.waiting)
  logWaitingTime({
...
export const deleteUser = (userId: number): Response<{}> => {
  ...
  metrics.deleteUserResponseTime.add(res.timings.waiting)
  logWaitingTime({
